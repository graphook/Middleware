...
export default function createType(scope, newType, path, options = {}) {
  return Promise.try(() => validateSchema(newType, type_type.properties, scope.errors, path))
    ...
    .then(() => createObjects(scope, [newType], path, 'createResponse', {
      type: Object.assign(type_type, {_type: 'type_type', _id: 'type_type'}),
    ...
    .then(() => {
      let mappingToSend = {};
      mappingToSend[scope.createResponse.items[0].index._id] = scope.esMapping;
      return request.put(process.env.ES_URL + '/object/_mapping/' + scope.createResponse.items[0].index._id).send(mappingToSend);
    })
    .catch((err) => { throw err })
}
